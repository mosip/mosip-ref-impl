#-- -------------------------------------------------------------------------------------------------
#-- Job Name        : ID Repository DB Tables Archive
#-- DB Name 	    : mosip_idrepo
#-- Table Names     : uin_document_h
#-- Purpose    	    : Job to Archive Data in ID Repository DB for above mentioned tables         
    ...
    try:
        ...
        if select_count > 0:
            for row in rows:
                ...
                if insert_count > 0:
                    delete_query = "DELETE FROM "+sschemaName+"."+tableName+" WHERE uin_ref_id ='"+row[0]+"'AND doccat_code='"+row[1]+"'AND eff_dtimes='"+str(row[3])+"'"
