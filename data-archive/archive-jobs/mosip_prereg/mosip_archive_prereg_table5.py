...
def dataArchive():
    ...
    try:
        ...
        tableName=dbparam["archive_table5"]
        sschemaName = dbparam["source_schema_name"]
        aschemaName = dbparam["archive_schema_name"]
        oldDays = dbparam["archive_older_than_days"]
        
        print(tableName)
        select_query = "SELECT * FROM "+sschemaName+"."+tableName+" WHERE application_status_code = 'BOOKED' AND cr_dtimes < NOW() - INTERVAL '"+oldDays+" days'"
