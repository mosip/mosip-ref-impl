...
import * as centerConfig from 'src/assets/entity-spec/partner.json';
...
export class ViewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.auditService.audit(3, centerConfig.auditEventIds[0], 'partnerservices');
  }
  ...
  pageEvent(event: any) {
    ...
    this.router.navigateByUrl(`admin/resources/partner/view?${url}`);
  }
      ...
      .subscribe(({ response, errors }) => {
        ...
        if (response != null) {
          ...
          if (response.partners !== null) {
            this.partnerservices = response.partners ? [...response.partners] : [];
          } else {
            this.noData = true;
          }
        } else {
